...
var conversationComponent = function () {
    ...
    var vkService = window.xhrService;
    ...
    function Conversations(mountNode) {
        ...
        document.getElementById("chart-form").addEventListener('submit', function (event) {
            ...
            sendMessage(id, message).then(function (res) {
                showUserMessages(id);
            });
        });
    ...
    Conversations.prototype.showDialogs = function () {
        ...
        vkService.getDialogs().then(function (res) {
            ...
            var idList = json.response.map(function (item) {
            ...
            _this2.renderDialogs(mergeDialogsInfo(res.response, dialogs));
        }).catch(function (err) {
            alert(err);
        });
    };
    ...
    Conversations.prototype.showUserMessages = function (uid) {
        ...
        vkService.getMessages(uid).then(function (res) {
            return res.json();
        }).then(function (res) {
            _this3.renderMesasges(res.response);
        });
     ...
     * @returns {Element}
     */
    var messageRender = function messageRender(item) {
     ...
     * @returns {Element}
     */
    var dialogRender = function dialogRender(item) {
        var div = document.createElement('div');

        if ((typeof item === 'undefined' ? 'undefined' : _typeof(item)) !== 'object') return div;
